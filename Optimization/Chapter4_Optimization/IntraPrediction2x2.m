function [ResImage, modeArray2x2] = IntraPrediction2x2(image)
%Input : block (5x5 block)
%           BorderArray (1x2 cell, first cell represents upper border, second is left border)
%Output : ResBlock (4x4 resblcok)
%              mode (0-3)
    [M, N, C] = size(image);
    block_size = 2;
    ResImage = zeros(M, N, C);
    modeArray2x2 = ones(M / block_size, N / block_size, 3) * 10;
    ...
    for c = 1:C
        for i = 1:block_size:M
            for j = 1:block_size:N
                ...
                modeArray2x2(floor(i / block_size) + 1, floor(j / block_size) + 1, c) = mode - 1;
