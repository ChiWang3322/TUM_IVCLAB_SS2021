...
DataName = 'video_codec_values_optimized.mat';
...
%  Output        : motion_vectors_indices (Motion Vector Indices, size: (height/8) x (width/8) x 1 )
    function [x, y, step, SAD] = LogarithmicSearch(center_loc, step, ref_image, block, cur_x, cur_y, offset)
        ...
        right_block = ref_image(center_loc(1)  : 2 :center_loc(1)  + 2 * M - 2,...,
                                            center_loc(2) + step  : 2 : center_loc(2) + step  + 2 * N - 2);
        SAD_current = sum(abs(current_block - block),'all');
        SAD_up = sum(abs(up_block - block),'all');
        SAD_bottom = sum(abs(bottom_block - block),'all');
        SAD_left = sum(abs(left_block - block),'all');
        SAD_right = sum(abs(right_block - block),'all');
        if cur_x == -offset
            SAD_left = Inf;
        elseif cur_x == offset
            SAD_right = Inf;
