...
%         k_training  = IntraEncode(lena_small, qScale, EOB, true);
...
for i = 1:length(scales_video)
    ...
    for j = 1:num_frames
        ...
        if j == 1
            ...
            % Train Huffmann code table using lena_small
            range = min(frame1_intra_encode) : max(frame1_intra_encode);
            lena_small_intra_encode  = IntraEncode(lena_small, qScale, EOB, true);
            pmf_lena_small_intra_encode = stats_marg(lena_small_intra_encode, range);
            [BinaryTree, HuffCode, BinCode, Codelengths]=buildHuffman(pmf_lena_small_intra_encode);
            % Huffmann encoding
            offset = -min(frame1_intra_encode) + 1;
        ...
        % Motion compensation
            ...
            previous_frame_yuv = ictRGB2YCbCr(frames_rec{j - 1});
            % Get motion vectors and error image
            motion_vector = SSD(previous_frame_yuv(:, :, 1), frame_yuv(:, :, 1));
            rec_frame_yuv = SSD_rec(previous_frame_yuv, motion_vector);
            ...
            if j == 2
                pmf_mv = stats_marg(motion_vector, 1:(SearchRange * 2 + 1)^2);
            ...
            % Reconstruct frame
            rec_frame = dec_error_image + SSD_rec(previous_frame_yuv, dec_MV);
...
%% Huffmann Table training
function [BinaryTree, HuffCode, BinCode, Codelengths] = TrainHuffTable(path, pmf_train, EOB, qScale)
...
%  Output        : rec_image (Reconstructed current image, YCbCr image)
    ...
    ref_image = padarray(ref_image, [search_range, search_range], 0, 'both');
    block_size = [8, 8];
    [M, N, ~] = size(rec_image);
    for m = 1:block_size(1):M
        for n = 1:block_size(2):N
            mv = motion_vectors(floor(m / 8) + 1, floor(n / 8) + 1);
            sz = [2 * search_range + 1, 2 * search_range + 1];
            [col, row] = ind2sub(sz, mv);
            col = col - search_range - 1;
            row = row - search_range - 1;
            ref_loc = [m, n] + [row, col] + [search_range search_range];
    ...
    function motion_vector_indice = BlockSSD(block, loc, ref_image)
        ...
        for x = -search_range:search_range
            for y = -search_range:search_range
                ...
                if SSE < MIN_SSE
                    MIN_SSE = SSE;
                    best_loc = [x + search_range + 1, y + search_range + 1];
                end
            end
        end
        %Change Matrix indice to linear indice
        motion_vector_indice = sub2ind([2 * search_range + 1, 2 * search_range + 1], best_loc(2), best_loc(1));
    end
